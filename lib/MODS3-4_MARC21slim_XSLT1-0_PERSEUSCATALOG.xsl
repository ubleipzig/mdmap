...
<!-- Name elements -->
	...
	<xsl:template match="mods:name[@type='personal'][mods:role/mods:roleTerm[@type='text']='creator']">
		<!-- PerseusCatalog Addition -->
		<xsl:param name="xlink" select="./@xlink:href"/>
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">100</xsl:with-param>
			<xsl:with-param name="ind1">1</xsl:with-param>
			<xsl:with-param name="subfields">
				<!-- PerseusCatalog Addition - Start -->
				<xsl:if test="./@authority='naf'">
					<xsl:if test="$xlink">
						<marc:subfield code="0">(LCCN) <xsl:value-of select="substring-before(substring-after($xlink, 'http://errol.oclc.org/laf/'), '.html')"/>
						</marc:subfield>
					</xsl:if>
				</xsl:if>
				<!-- PerseusCatalog Addition - End -->
				<marc:subfield code="a">
					<xsl:value-of select="mods:namePart"/>
				</marc:subfield>
				<!-- v3 termsOfAddress -->
...
<!-- Identifiers -->	
	...
	<!-- PerseusCatalog Addition - Start -->
	<xsl:template match="mods:identifier[@type='ark' or @type='bhl' or @type='ctsurn' or @type='oca' or @type='oclc' or @type='Perseus' or @type='Perseus:abo' or @type='phi' or @type='stoa author-text' or @type='tlg' and .!='none' and .!='None']">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">024</xsl:with-param>
